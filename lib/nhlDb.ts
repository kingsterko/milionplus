...
/**
 * Uplne samostatna databazova vrstva pre NHL - vlastny bank, vlastne tipy,
 * ziadne miesanie s futbalovymi datami. Rovnaky vzor ako db.ts, len nad
 * nhl_bankroll_log / nhl_tips tabulkami.
 */

const DEFAULT_BANK = 10.0;

export interface NhlTip {
  id: number;
  placed_at: string;
  match: string;
  market: string;
...
export interface NhlBankrollEntry {
  timestamp: string;
  bank: number;
  note: string | null;
}
...
export async function getCurrentBank(): Promise<number> {
  ...
  if (!data) {
    await setBank(DEFAULT_BANK, "počiatočný NHL bank");
    return DEFAULT_BANK;
  }
  return data.bank;
}
...
export async function getBankrollHistory(): Promise<NhlBankrollEntry[]> {
  const supabase = getSupabase();
  const { data, error } = await supabase
    .from("nhl_bankroll_log")
...
export async function listOpenTips(): Promise<NhlTip[]> {
  const supabase = getSupabase();
  const { data, error } = await supabase
    .from("nhl_tips")
    .select("*")
...
export async function listAllTips(limit = 50): Promise<NhlTip[]> {
  const supabase = getSupabase();
  const { data, error } = await supabase.from("nhl_tips").select("*").order("id", { ascending: false }).limit(limit);
  if (error) throw error;
  return data ?? [];
}
...
export async function settleTip(tipId: number, won: boolean): Promise<void> {
  const supabase = getSupabase();
  const { data: tip, error: fetchErr } = await supabase.from("nhl_tips").select("*").eq("id", tipId).single();
...
export async function deleteTip(tipId: number): Promise<void> {
  const supabase = getSupabase();
  const { error } = await supabase.from("nhl_tips").delete().eq("id", tipId).eq("status", "open");
  if (error) throw error;
}
...
export async function updateTip(tipId: number, updates: { odds?: number; stake?: number }): Promise<void> {
  ...
  const { error } = await supabase.from("nhl_tips").update(patch).eq("id", tipId).eq("status", "open");
  if (error) throw error;
}
